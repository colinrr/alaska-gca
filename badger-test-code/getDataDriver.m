...
clear; close all;
global homedir
homedir = '/home/crowell/'; %activate_base;
...
wPreProcess  = false; % Apply pre-processing
responseFlag = false; % Remove response? 
mtSpecFlag   = false;
filterFlag   = false;

savchans     = false; % Save channel response info (only if downloaded)
savspecs     = false; % Save spectrogram information

% Plot flags
mapChannelArray = 0; % 0 = no map, 1 = stations only, 2 = stations plus event location
mapDataArray    = 0; % Same as above, but only maps stations with data for event - will overwrite files for above
plotRawWavs     = 1; % Plot raw waveforms
...
plotMTspecs     = 0; % Plot multitaper spectrograms
...
oMat = fullfile(paths.raw,[fname '.mat']);
oPs  = fullfile(paths.inven,[fname '.ps']);
